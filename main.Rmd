...
```{r setup, eval = FALSE}
!install.packages(c("httr", "dplyr", "jsonlite", "progress", "dplyr", "tidyr", "ggplot2", "knitr", "ggcorplot"))
```
...
```{r}
ggplot(movies, aes(x = season, y = revenue, fill = season)) +
...
genre_revenue <- movies %>%
  select(starts_with("Action"):starts_with("War"), revenue) %>%
  pivot_longer(cols = starts_with("Action"):starts_with("War"), names_to = "Genre", values_to = "Presence") %>%
  filter(Presence == 1)
...
## continue visualizing the rest predictors and additional?
